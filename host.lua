return function(...)
    local data = ProccessArgs(...)  -- Note: "ProccessArgs" might be a typo, should it be "ProcessArgs"?
    ...
    if data[2]:lower() == "line" then
        ...
        local BaseAnchor = game:GetService("Players"):GetPlayerByUserId(Settings['host'])
        local Set = CreateSet()
    ...
    elseif data[2]:lower() == "form" then
        ...
        local function CreateSet()
            local Cords = {}
            for i = 1, 40 do
                local C = string.format("%d,0,%d", FormSettings.CurrentX, FormSettings.CurrentZ)
                Cords[i] = C
        ...
        for i, v in pairs(NewAlts) do
            local cords = Cords[i]
            local x, y, z = unpack(cords:split(","))
            if v == player.UserId then
                player.Character.HumanoidRootPart.CFrame = BaseAnchor.Character.HumanoidRootPart.CFrame * CFrame.new(tonumber(x), tonumber(y), tonumber(z))
                break
            end
        end
    ...
    elseif data[2]:lower() == "moneyheart" then
        if ReSort(Alts)[1] == player.UserId then
            ...
            for _, v in pairs(workspace:GetChildren()) do 
                if v.Name:find("temp") then 
                    v:Destroy() 
                end 
            end

            for _, v in pairs(Cords) do
                local x, y, z = unpack(v:split(","))
                local Part = Instance.new("Part", workspace)
                Part.Name = "temp"
                Part.Size = Vector3.new(1, 5, 1)
                Part.Transparency = 1
                Part.CanCollide = false
                Part.CFrame = player.Character.HumanoidRootPart.CFrame * CFrame.new(tonumber(x), tonumber(y), tonumber(z))
